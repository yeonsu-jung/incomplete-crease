...
% azimuthal sampling
delta_R = 1;
min_r = 350; % cutoff for visualization
num_R = 20;
...
centered = GetPointCloudFrom3dImage(zstack);
centered = RotatePoints(centered,rotation_angle);

% binning and averaging...
[az,el,r] = cart2sph(centered(:,1),centered(:,2),centered(:,3));

close all;
PlotContours(centered, r, min_r, max(r), num_R, delta_R);
close all;
PlotThetaPsi(az,el,r, min_r, max(r), num_R, delta_R);
...
%% Vectorized approach
for i = 1:num_R
    I_R = abs(r .* cos(el) - R_list(i)) < delta_R;  % Logical indexing for fixed R
    
    for j = 1:num_az
        % Calculate differences and use logical indexing
        I = mod(abs(az - az_bin(j)), 2*pi) < delta_az;
        combined_I = I_R & I;
        
        if any(combined_I)
            % Vectorized operations to compute mean, min, and max
            el_values = el(combined_I);
            binned_data(j, i) = mean(el_values);
            binned_data_min(j, i) = min(el_values);
            binned_data_max(j, i) = max(el_values);
        end
    end
end

%%
% i_start, i_end determined by visual inspection...
i_start = 1;
i_end = 20;
...
% that's why I made those stupid codes...
